...
def run_parallel_blind_validation():
    print("=" * 75)
    print("  PARALLELIZED BLIND LSTM FORECAST VALIDATION FOR ALL TXSON STATIONS")
    print("=" * 75)
    ...
    df_filtered = df_raw[df_raw['Date'] <= gt_cutoff].dropna(subset=['Precipitation', 'T_Max', 'T_Min', 'SMAP_Moisture']).sort_values('Date')
    
    # Training Cutoff: 2020-09-01 (1 year prior to ground truth cutoff)
    ...
    # 2. Scale Features & Build Training Sequences
    ...
    # 4. Parallel Processing Across All Stations
    ...
    df_metrics.to_csv('txson_all_stations_blind_validation_metrics.csv', index=False)
    ...
    print("=" * 55)
    print("\n[+] Saved validation summary to 'txson_all_stations_blind_validation_metrics.csv'")
    ...
    fig.suptitle("TxSON Capstone Demonstration: 1-Year Blind LSTM Forecast vs Ground Truth (All 6 Stations)\nEvaluation Period: 2020-09-01 to 2021-09-01 (Zero-Leakage)", fontsize=14, fontweight='bold', y=0.98)
