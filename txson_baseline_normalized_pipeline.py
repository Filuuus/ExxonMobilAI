...
def run_baseline_normalized_pipeline():
    print("=" * 80)
    print("  TxSON PER-STATION BASELINE NORMALIZED RECURSIVE LSTM PIPELINE")
    print("  Resolving Spatial Heterogeneity, Soil Baselines & R2 Degeneration")
    print("=" * 80)
    
    # 1. Ingest unified expanded dataset
    dataset_path = 'datasets/TxSON_Expanded_Unified_Dataset.csv'
    ...
    weather_features = ['Precipitation', 'T_Max', 'T_Min']
    ...
    model.save("baseline_normalized_lstm_model.keras")
    print("[+] Saved model to 'baseline_normalized_lstm_model.keras'.")
    ...
    for st_id in active_test_stations:
        ...
        for step in range(horizon):
            lstm_in = curr_seq.reshape(1, look_back, 4)
            ...
            next_step = np.zeros((1, 4))
            next_step[0, 0:3] = test_w[step, 0:3]
            next_step[0, 3] = pred_z
    ...
    metrics_csv = "txson_baseline_normalized_metrics.csv"
    df_metrics.to_csv(metrics_csv, index=False)
    print(f"\n[+] Saved baseline-normalized metrics to: '{metrics_csv}'")
    ...
    for idx, st_id in enumerate(plot_stations):
        ...
        ax.plot(res['Dates'], res['Forecast'], label='Baseline-Normalized Forecast', color='#2ca02c', linestyle='--', linewidth=1.8)
    ...
    fig.suptitle(
        f"TxSON Per-Station Baseline-Normalized 1-Year Blind Forecast vs In-Situ Ground Truth\n"
        f"Evaluation Period: 2020-09-01 to 2021-09-01 (Continuous 365-Day Zero-Leakage Rollout)",
        fontsize=14, fontweight='bold', y=0.99
    )
    plt.tight_layout(rect=[0, 0.03, 1, 0.96])
    
    plot_out = "txson_baseline_normalized_validation.png"
